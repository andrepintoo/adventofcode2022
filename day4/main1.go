...
func main(){
	...
	for scanner.Scan() {
		...
		// fmt.Printf("Line %s, left1 %d, right1 %d, left2 %d, right2 %d\n", line, leftE1, rightE1, leftE2, rightE2)
		if leftE1 <= leftE2 && rightE1 >= rightE2 {
			count++
			continue
		}

		if leftE2 <= leftE1 && rightE2 >= rightE1 {
			count++
		}
	}

	fmt.Printf("Number of pairs that fully contain a range: %d\n", count)
